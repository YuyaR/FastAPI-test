...
async def read_item_list(skip: int = 0, limit: int = 100):
	items = crud.read_item_list(db=Depends(get_db), skip=skip, limit=limit)
	if items is None:
		raise HTTPException(
			status_code=404,
...
async def read_item(item_q: Union[int, str]):
	try:
		int(item_q)
		items = crud.read_item_by_id(db=Depends(get_db), item_id=int(item_q))
	except ValueError:
		items = crud.read_item_by_name(db=Depends(get_db), name=item_q)
	if items is None:
...
async def create_item(
	...
	return {
		...
		"item_info": crud.create_item(db=Depends(get_db), item=item)
	}

@app.delete("/items/", status_code=202)
async def delete_item(item: schema.ItemDel):
	crud.remove_item(db=Depends(get_db), item=item)
